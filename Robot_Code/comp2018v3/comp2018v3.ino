...
#define LEFT_BASE_START 80
#define LEFT_BASE_COUNT 14
#define NAME_BOTTOM_START 94
#define NAME_BOTTOM_COUNT 9
#define BLINKER_START 103
#define BLINKER_COUNT 1
#define NAME_TOP_START 104
#define NAME_TOP_COUNT 9
#define RIGHT_BASE_START 113
...
void towerLights(int ledNum)
{
  switch (tower)
  {
    ...
    case 4: //elevator with cube is going down.
      cascadeBack(RED, ledNum);
      break;
    case 5: //elevator with cube is going up
      cascadeFor(GREEN, ledNum);
      break;
    case 6:
...
void cascadeFor(unsigned long color, int ledNum)
{
  clearLEDs();  // Turn off all LEDs
  leds.setPixelColor(ledNum, color);  // Set just this one
  leds.show();
}
void cascadeBack(unsigned long color, int ledNum)
{
  clearLEDs();  // Turn off all LEDs
  leds.setPixelColor(RIGHT_TOWER_START + RIGHT_TOWER_COUNT - ledNum, color);  // Set just this one
  leds.show();
}
void clearLEDs()
{
  for (int i = 0; i < RIGHT_TOWER_START + RIGHT_TOWER_COUNT; i++)
  {
