...
void rainbow(byte startPosition)
{
  ...
  int rainbowScale = 192 / (.5 * LED_COUNT);
  ...
  for (int i = 0; i < LED_COUNT; i++)
  {
    ...
    int orderNum = (rainbowScale * (i + startPosition)) % 192;
...
uint32_t rainbowOrder(byte position)
{
  ...
  else if (position < 191) //160 <position< 191   Fuchsia->Red (Red = FF, green = 0, blue goes FF->00)
  {
    position -= 159;
